...
function(){$(this).addClass("cover-animate").addClass(hoverClasses.to)})},_self.options.hoverDelay);if($hoverElem.find("i").hasClass("align-center"))$hoverElem.find("i").centerElement()}else{$hoverElem.addClass("cover-animate");clearTimeout(_self.tmhover);$hoverElem.addClass(hoverClasses.from)}})},_getDir:function($el,coordinates){var w=$el.width(),h=$el.height(),x=(coordinates.x-$el.offset().left-w/2)*(w>h?h/w:1),y=(coordinates.y-$el.offset().top-h/2)*(h>w?w/h:1),direction=Math.round((Math.atan2(y,
x)*(180/Math.PI)+180)/90+3)%4;return direction},_getClasses:function(direction){var fromClass,toClass;switch(direction){case 0:!this.options.reverse?fromClass="cover-slideFromTop":fromClass="cover-slideFromBottom";toClass="cover-slideTop";break;case 1:!this.options.reverse?fromClass="cover-slideFromRight":fromClass="cover-slideFromLeft";toClass="cover-slideLeft";break;case 2:!this.options.reverse?fromClass="cover-slideFromBottom":fromClass="cover-slideFromTop";toClass=
"cover-slideTop";break;case 3:!this.options.reverse?fromClass="cover-slideFromLeft":fromClass="cover-slideFromRight";toClass="cover-slideLeft";break}return{from:fromClass,to:toClass}}};var logError=function(message){if(this.console)console.error(message)};$.fn.hoverdir=function(options){if(typeof options==="string"){var args=Array.prototype.slice.call(arguments,1);this.each(function(){var instance=$.data(this,"hoverdir");if(!instance){logError("cannot call methods on hoverdir prior to initialization attempted to call method _");
